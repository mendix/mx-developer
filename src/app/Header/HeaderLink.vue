<template>
  <div :class="b()" @mouseenter="mouseenter" @mouseleave="mouseleave" @click="menu">
    <a :class="b('head', { on, 'has-sub': link.sub && link.sub.length > 0 })" v-if="link.url" :href="link.url" :title="link.title">{{ link.title }}</a>
    <a :class="b('head', { on, 'has-sub': link.sub && link.sub.length > 0 })" v-else href="#" :title="link.title" @click.stop.prevent="menu">{{ link.title }}</a>
