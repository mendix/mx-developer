...
const onSprintr = () => sprintrRegEx.test(window.location.origin);
const onCloud = () => cloudRegEx.test(window.location.origin);

const onHeimdal = () => heimdDalRegEx.test(window.location.origin);
const onBrokkr = () => brokkrRegEx.test(window.location.origin);

const onAppStore = () => appstoreRegEx.test(window.location.origin);
const onBeaver = () => beaverRegEx.test(window.location.origin);
...
const waitForElementId = (id, Component, store, num = 200) => {
    const el = document.getElementById(id);
    if (el !== null) {
...
const waitForElementClass = (className, Component, store, num = 200) => {
